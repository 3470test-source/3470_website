...
const courses = {
  "Medical Coding Training": { fee: 10000, discount: 9000, paymentLink: "https://rzp.io/rzp/20KZ6Mj" },
  "Basic Medical Coding Training": { fee: 14000, discount: 2000, paymentLink: "https://rzp.io/rzp/20KZ6Mj" },
  "Advanced Medical Coding Training": { fee: 14000, discount: 2000, paymentLink: "https://rzp.io/rzp/20KZ6Mj" }
};
...
app.post("/api/enquiry", async (req, res) => {
  try {
    ...
    const { fee, discount, paymentLink } = courses[course];
    ...
    const [result] = await pool.query(`
      ...
      (name,email,phone,course,location,message,final_amount,razorpay_link_id,status)
      VALUES (?,?,?,?,?,?,?,?,'pending')
    `, [name, email, phone, course, location, message || "NA", finalAmount, paymentLink]);

    const enquiryNo = `3470-${result.insertId}`;
    await pool.query(`UPDATE enquiries_3470_data SET enquiry_no=? WHERE id=?`, [enquiryNo, result.insertId]);

    // Send email to user with payment link
    await transporter.sendMail({
      from: `"3470 HealthCare" <${process.env.GMAIL_USER}>`,
      to: email,
      subject: `Payment Link for Your Course – ${course}`,
      html: `
        <div style="font-family:Arial;padding:20px;border:1px solid #e5e5e5;border-radius:8px;">
          <h3>Hello ${name},</h3>
          <p>Your payment link for <b>${course}</b> is ready.</p>
          ...
          <a href="${paymentLink}" target="_blank"
             style="display:inline-block;padding:12px 24px;background-color:#068545;color:#fff;text-decoration:none;border-radius:4px;">
            Pay Now
          </a>
        </div>
      `
    ...
    res.json({
      success: true,
      message: "Enquiry submitted successfully! ✅ Payment link sent to your email."
    });
...
app.post("/api/confirm-payment", async (req, res) => {
  try {
    const { email, invoicePdfUrl } = req.body;
    ...
    const [result] = await pool.query(`
      UPDATE enquiries_3470_data
      SET status='paid', user_id=?
      WHERE email=? AND status='pending'
    `, [userId, email]);
    
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "No pending enquiry found for this email" });
    }
